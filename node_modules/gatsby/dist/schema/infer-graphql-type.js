...
function findLinkedNode(value, linkedField) {
...
function inferFromFieldName(value, key, selector, types) {
    ...
    if (fields.length > 1) {
      ...
      if (!type) {
        type = new GraphQLUnionType({
          ...
          description: `Union interface for the field "${key}" for types [${linkedTypes.sort().join(`, `)}]`,
...
function _inferObjectStructureFromNodes({
  ...
  _.each(resolvedExample, (value, key) => {
    ...
    if (mapping && _.includes(Object.keys(mapping), fieldSelector)) {
      inferredField = inferFromMapping(value, mapping, fieldSelector, types); // Second if the field has a suffix of `___NODE`, we use the field value
