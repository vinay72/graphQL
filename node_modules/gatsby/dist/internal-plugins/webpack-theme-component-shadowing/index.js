...
module.exports = class GatsbyThemeComponentShadowingResolverPlugin {
  ...
  resolveComponentPath({
    ...
    if (!this.cache[`${theme}-${component}`]) {
      this.cache[`${theme}-${component}`] = [path.join(path.resolve(`.`), `src`, theme)].concat(themes.map(aTheme => path.join(path.dirname(require.resolve(aTheme)), `src`, theme))).map(dir => path.join(dir, component)).find(possibleComponentPath => {
